...
function send_password_reset($get_name,$get_email,$token){
    ...
    <br/><br/>
    <a href='http://localhost/teacher/password_change.php?token=$token&email=$get_email'>Click me</a>
    ";
...
if(isset($_POST['password_reset_link'])){
    ...
    $check_email = "SELECT name,email FROM faculty WHERE email='$email' LIMIT 1";
    ...
    if(mysqli_num_rows($check_email_run) > 0){
        ...
        $update_token = "UPDATE faculty SET verify_token='$token' WHERE email='$get_email' LIMIT 1";
...
if(isset($_POST['password_update'])){
    ...
    if(!empty($token)){
        if(!empty($email) && !empty($new_password) && !empty($confirm_password)){
            ...
            $check_token = "SELECT verify_token FROM faculty WHERE verify_token='$token' LIMIT 1";
            $check_token_run = mysqli_query($conn, $check_token);
            if(mysqli_num_rows($check_token_run) > 0){

                if($new_password==$confirm_password){
                    $new_password_hash=password_hash($new_password,PASSWORD_DEFAULT);
                    $update_password = "UPDATE faculty SET password ='$new_password_hash' WHERE verify_token='$token' LIMIT 1";
                    $update_password_run = mysqli_query($conn, $update_password);

                    if($update_password_run){

                        $new_token = md5(rand())."guru";
                        $update_to_new_token = "UPDATE faculty SET verify_token='$new_token' WHERE verify_token='$token' LIMIT 1";
